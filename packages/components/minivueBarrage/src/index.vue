<template>
  <div
    ...
    :class="bindDynamicClass"
    ref="barrageWapperRef"
    :style="barrageWapperStyle"
  >
...
export default defineComponent({
  ...
  setup(props, { slots, expose, emit }) {
    ...
    const bindDynamicClass = computed(() =>
      props.defaultBgColor ? "default-background-color" : "",
    );
    /**
    ...
    const maxLineCount = computed(() => {
      const paddingTopValue = Number(
        unitToValue(getStyleValue(barrageWapperRef.value!, "paddingTop")),
      );
      const paddingBottomValue = props.fullScreen
        ? Number(unitToValue(getStyleValue(barrageWapperRef.value!, "paddingBottom")))
        : 0;
      const paddigHeight = paddingTopValue + paddingBottomValue;
      const count =
        (barrageWapperRef.value?.clientHeight &&
          Math.floor((barrageWapperRef.value?.clientHeight - paddigHeight) / baseHeight)) ||
        0;
    ...
    const clearData = () => {
      ...
      lastIndex = -1;
      (lastRecordIndex = -1), (clientWidthList = []);
    ...
    const create = (instance: BarrageItem) => {
      ...
      // 找到最小的数字弹道 进行添加
      let minIndex =
        clientWidthList.findIndex((item) => item === Math.min(...clientWidthList)) || 1;
      if (minIndex === lastIndex) {
        //如果 刚刚添加的位置 跟此 次查找的最小距离位置 相等 ，则 再找一个更合适的
        minIndex = clientWidthList.findIndex(
          (item, index) => item === Math.min(...clientWidthList) && index !== lastIndex,
        );
      }
      lastIndex = minIndex;
    ...
    const batchRemoveBarrageEl = () => {
      const allBarrages = [
        ...(topWapperRef.value?.childNodes || []),
        ...(bottomWapperRef?.value?.childNodes || [])!,
      ] as HTMLDivElement[];
      if (!allBarrages.length) return; // 如果没有 子节点 则不操作
      const awaitDestoryEls = allBarrages.filter(
        (item) => item.getAttribute(KEYGROUP.RUNNINGSTATE) === PLAYSTATEGROUP.END,
      ); // 等待被批量删除的 弹幕元素数组
    ...
    const pausedAllBarrage = () => {
      ...
      for (const el of allBarrages) {
        el.style.animationPlayState = props.pausedFlag
          ? PLAYSTATEGROUP.PAUSED
          : PLAYSTATEGROUP.RUNNING;
      }
      if (props.pausedFlag) {
    ...
    const appendElement = (currentRowIndex: number) => {
      ...
      currentRowIndex >= maxLineCount.value / 2
        ? bottomWapperRef?.value?.appendChild(barrageElement)
        : topWapperRef.value?.appendChild(barrageElement);
      barrageElement.appendChild(barrageIconElement);
    };
    /**
    ...
    const elAddEventListener = (instance: BarrageItem) => {
      ...
      const mouseLeaveCallback = (e: MouseEvent) => {
        ...
        !props.pausedFlag &&
          (currentMouseLeaveDom.style.animationPlayState = PLAYSTATEGROUP.RUNNING);
      };
      ...
      const animationendCallback = (e: AnimationEvent) => {
        ...
        if (
          curentFinishRunningNum === baseBatchDestoryNum ||
          cacheBarrageRenderNum - finishBatchDestroyNum < baseBatchDestoryNum
        ) {
    ...
    const calcAppendLineIndex = (): number =>
      clientWidthList.findIndex((item) => item === Math.min(...clientWidthList));

    function toScriptCreateBarrageItem(itemInstance: BarrageItem, currentRowIndex: number) {
      const top =
        currentRowIndex === 0
          ? props.baseLineHeight
          : baseHeight * currentRowIndex + props.baseLineHeight;
    ...
    function setElementStyleAttrs(instance: BarrageItem, index: number) {
      ...
      elStyle.setProperty(
        "--wapperClientWidth",
        `-${(barrageWapperRef.value?.clientWidth || 0) + offsetRightValue}px`,
      );
