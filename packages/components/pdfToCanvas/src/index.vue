...
import { base64ToBytes, blobToArrayBuffer } from "../../../utils";
import * as pdfjsLib from "pdfjs-dist";
import * as pdfjsWorker from "pdfjs-dist/legacy/build/pdf.worker.mjs";
const { pdfCanvasRef, sentHttpRequestGetPdf } = usePdfToCanvas();
// 定义 props
const props = withDefaults(defineProps<vuePdfToCanvasProps>(), defaultProps);
const pdfStreamData = ref<any>();
let pdfDocInstance: pdfjsLib.PDFDocumentProxy = {} as any;
const maxPage = ref(1);
const getPdf = async () => {
  if (props.url) {
    // 如果传入的是url 则发送请求获取数据 默认get 发送请求获取pdf
    const result = await callWithErrorHanding(async () => await sentHttpRequestGetPdf(props.url));
...
const renderPdfPage = async (pageNumber: number) => {
  ...
  if (maxPage.value > pageNumber) {
    renderPdfPage(pageNumber + 1);
  }
};
// 转换Stream流
const tramsformStream = () =>
  typeof pdfStreamData.value === "string"
    ? base64ToBytes(pdfStreamData.value)
    : blobToArrayBuffer(pdfStreamData.value);
...
const pdfStreamHandle = async () => {
  // 转换流为 arrayBuffer
  const arrayBuffer = await tramsformStream();
  ...
  maxPage.value = pdfDocInstance._pdfInfo.numPages; // 存储最大页数
